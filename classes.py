...
class cliente:
    def __init__ (self, cpf, nome_completo, data_nascimento, endereco, telefone, cep):
        ...
        self.endereco= f"{endereco_dict.get('street', '')}, {endereco_dict.get('neighborhood', '')}, {endereco_dict.get('city', '')} - {endereco_dict.get('state', '')}"
    def __str__ (self) :
...
def clientes():
    ...
    with open("clientes.txt", "r") as arquivo:
        for linha in arquivo:
            ...
            endereco = partes[5].replace("Endereço:", "").strip()
...
def clientes_salvar(clientes):
    with open("clientes.txt", "w") as arquivo:
            for c in clientes:
                arquivo.write(f" Dados do cliente: CPF: {c.cpf} || Nome completo: {c.nome_completo} || Data de nascimento: {c.data_nascimento} || Telefone: {c.telefone} || CEP: {c.cep} || Endereço: {c.endereco}\n")
